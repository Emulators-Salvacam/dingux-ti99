...
# define MAX_MENU_JOY_ITEM (MENU_JOY_BACK + 1)
  ...
  static int ti99_auto_fire_mode   = 0;
...
psp_display_screen_joystick_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_JOY_ITEM; menu_id++) {
    ...
    if (menu_id == MENU_JOY_AUTOFIRE_M) {
      if (ti99_auto_fire_mode) strcpy(buffer,"yes");
      else                    strcpy(buffer,"no ");
      string_fill_with_space(buffer, 4);
      psp_sdl_back2_print(140, y, buffer, color);
      y += y_step;
...
psp_joystick_menu_init(void)
{
  ...
  ti99_auto_fire_mode   = TI99.ti99_auto_fire;
}
...
psp_joystick_menu_validate(void)
{
  ...
  if (ti99_auto_fire_mode != TI99.ti99_auto_fire) {
    kbd_change_auto_fire(ti99_auto_fire_mode);
  }
}
  ...
  while (! end_menu)
  {
        ...
        (new_pad == GP2X_CTRL_CIRCLE))
    {
      ...
      switch (cur_menu_id ) 
      {
        ...
        break;              
        case MENU_JOY_AUTOFIRE_M  : ti99_auto_fire_mode = ! ti99_auto_fire_mode;
        break;              
